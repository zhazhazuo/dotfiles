...
description: Use when the user has completed a feature development task and the feature is not yet documented in ./repoWiki/06_features/
...
- AND `./repoWiki/06_features/<feature-name>.md` does not yet exist
...
4. Update ./repoWiki/RUNBOOK.md after feature and index files, then update ./repoWiki/META.md last
```
...
Using it ensures the output matches existing `./repoWiki/06_features/` files in format.
