...
assert_file_contains "$THEME" 'set-hook -g pane-exited '\''run-shell -b "~/dotfiles/tmux/scripts/agent-monitor-prune.sh"'\''' 'pane exit prunes agent monitor records'
assert_file_contains "$THEME" 'set-hook -g after-kill-pane '\''run-shell -b "~/dotfiles/tmux/scripts/agent-monitor-prune.sh"'\''' 'pane kill prunes agent monitor records'
